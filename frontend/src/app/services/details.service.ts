...
export class DetailsService {
    ...
    let detail$ = this.http.get(this.rootURL + '/detail/' + ticker)
      .pipe(
        tap( (res: DetailsResponse) => {
          console.log("the respronse that i recieved from backend is ",res);
          if (!res.success) {
            res.results = [];
            return res;
          }
          res.results = res.results.map(
            detail => new CompanyDetails(detail.ticker, detail.name, 
                detail.description, detail.startDate, detail.exchangeCode,detail.closingprice,detail.currentprice));
                return res;
      }));
  ...
  public getDailyChart(ticker: string, startDate: string): Observable<DailyChartResponse> {
    ...
    .pipe(
      tap( (res: DailyChartResponse) => {
        ...
        console.log("the daily chart of the company is" ,res);
        return res;
    }));  
  }
  ...
  public getStocks(): Observable<CompanyDetails[]> {

    return this.http.get<CompanyDetails[]>(this.rootURL + '/stocks')
    
    .pipe(
      tap( (res: CompanyDetails[]) => {
        console.log("the data from backend is",res)
        return res;
    }));  
  }
    ...
    let news$ = this.http.get<NewsResponse>(this.rootURL + '/news/' + ticker)
      .pipe(
        tap( (res: NewsResponse) => {
          ...
          console.log("the news about the getNews and his chart is ",res);
          return res;
      }));
    ...
    let hist$ = this.http.get(this.rootURL + '/chart/historical/' + ticker)
      .pipe(
        tap( (res: HistChartResponse) => {
          ...
          console.log("the hostorical chart is",res);
          return res;
      }));
